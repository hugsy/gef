...
def generate_changelog(args: argparse.Namespace) -> bool:
    ...
    for author in contributor_names:
        author_commits = shell(
            f'git log {latest_tag}..HEAD  --pretty=format:"%h" --author="{author}"'
        ).splitlines()
...
"""
    ...
    if args.push_release:
        shell(
            f"""git tag --annotate "{args.version}" --message "Release {args.version} - {args.codename}" --sign"""
        )
...
def test_token(args) -> bool:
    ...
    if req.status_code != 200:
        ...
        print(
            f"Get a token from {GITHUB_NEW_TOKEN_URL} with 'Read-Only' privilege on Issues for the {args.repository} repository"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        usage=__usage__, description=__desc__, prog=__file__
    )
    parser.add_argument(
        "--debug", action="store_true", default=DEBUG, help="Enable debug output"
    )
    parser.add_argument(
        "-r",
        "--repository",
    ...
    parser.add_argument(
        ...
        help="Specify the Github token to use (requires `repo` scope)",
    )
    parser.add_argument(
        "-o",
        "--output-file",
